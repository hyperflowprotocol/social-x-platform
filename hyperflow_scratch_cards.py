#!/usr/bin/env python3
"""
HyperFlow Scratch Cards - HYPE Token Platform
Instant-win scratch card game with animated reveals and multiple card types
Features: Different card rarities, bonus multipliers, collection system
"""
...
<head>
    ...
    <title>HyperFlow Scratch Cards - HYPE Token Game</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card-type {
            ...
            backdrop-filter: blur(10px); text-align: center; cursor: pointer;
            transition: all 0.3s ease; border: 2px solid transparent;
        }
        .card-type:hover { transform: translateY(-5px); border-color: #00d4ff; }
        ...
        .win-rate { font-size: 1rem; color: #28a745; margin: 10px 0; }
        ...
        .win-details { display: flex; flex-direction: column; }
        .win-player { font-weight: bold; }
        .win-type { color: #B0B0B0; font-size: 0.9rem; }
        .win-amount { font-size: 1.2rem; font-weight: bold; color: #00d4ff; }
        
        @media (max-width: 768px) {
            .cards-grid { grid-template-columns: 1fr; }
            .scratch-card { padding: 20px; }
        }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1><span class="hype-logo"></span>HyperFlow Scratch Cards<span class="hype-logo"></span></h1>
            <p>Instant Win HYPE Token Games • HyperEVM Ecosystem</p>
        </div>
    ...
    <div class="scratch-area" id="scratch-area">
        <div class="scratch-card">
            ...
            <div style="background: linear-gradient(45deg, #00d4ff, #0099cc); color: white; padding: 15px; border-radius: 10px; margin-bottom: 20px;">
                ...
                <p>Drag to scratch - match 3 to win!</p>
            </div>
    ...
    <script>
        ...
        function buyCard(type) {
            const costs = {bronze: 100, silver: 500, gold: 2000, diamond: 10000};
            const cost = costs[type];
            
            if (confirm(`Buy ${type} card for ${cost} HYPE?`)) {
                // In production, this would deduct HYPE from wallet
                scratchCard(type);
            }
        }
        ...
        function continueScratch(event, overlay) {
            ...
            overlay.style.background = `radial-gradient(circle at ${x}px ${y}px, transparent 20px, 
                linear-gradient(45deg, #c0c0c0, #e8e8e8, #a8a8a8) 25px)`;
            
            // If scratched enough (simulated by mouse movement), reveal
        ...
        function scratchSymbol(overlay, index) {
            ...
            setTimeout(() => {
                ...
                const symbol = overlay.parentElement.querySelector('span');
                ...
                if (data.recent_wins.length > 0) {
                    winsList.innerHTML = data.recent_wins.map(win => `
                        <div class="win-item">
                            ...
                            <div class="win-amount">${win.prize.toLocaleString()} HYPE ${win.bonus ? '🔥' : ''}</div>
                        </div>
                    `).join('');
                } else {
        ...
        console.log('🎫 HyperFlow Scratch Cards Loaded');
    </script>
</body>
</html>"""
            ...
            # Parse card type from query
            card_type = 'bronze'
            if '?type=' in self.path:
                card_type = self.path.split('?type=')[1]
            
            # Generate card result
            result = scratch_game.generate_card_result(card_type)
            scratch_game.stats['total_cards_sold'] += 1
...
if __name__ == "__main__":
    print("🎫 HyperFlow Scratch Cards - HYPE Token Platform")
    print("=" * 60)
    print("💎 Features:")
    print("  - 4 Card Types: Bronze, Silver, Gold, Diamond")
    print("  - Instant Win with Animated Reveals")
    print("  - Bonus Multipliers up to 5x")
    print("  - Live Statistics and Recent Winners")
    print("=" * 60)
    
    try:
        with socketserver.TCPServer(("0.0.0.0", PORT), ScratchCardHandler) as httpd:
            print(f"🌐 HyperFlow Scratch Cards: http://localhost:{PORT}")
            print("🎫 Scratch card game loading...")
            httpd.serve_forever()
    except OSError as e:
        if e.errno == 98:
            print(f"Port {PORT} in use, trying alternative...")
            for alt_port in [7001, 7002, 8000, 8001]:
                try:
                    with socketserver.TCPServer(("0.0.0.0", alt_port), ScratchCardHandler) as httpd:
                        print(f"🌐 HyperFlow Scratch Cards: http://localhost:{alt_port}")
                        print("🎫 Scratch card game loading...")
                        httpd.serve_forever()
                except OSError:
                    continue
